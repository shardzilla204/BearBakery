...
public partial class MixerSlot : HBoxContainer
{
    ...
    public override void _Process(double delta)
    {
		...
		{
			UpdateMixingBar(BearBakery.OvenManager.CookColor);
		}
    }
    ...
    public override Variant _GetDragData(Vector2 atPosition)
    {
        ...
        Control dragPreview = BearBakery.GameManager.GetDragPreview(_bowl);
    ...
    public override void _DropData(Vector2 atPosition, Variant data)
    {
        ...
        if (BearBakery.Recipes.HasRecipe(_bowl.Ingredients))
        {
            Ingredient product = BearBakery.Recipes.GetProduct(_bowl.Ingredients);
            if (product == null)
            {
    ...
    private void AddMixTimer()
    {
        ...
        {
            WaitTime = BearBakery.MixerManager.MixWaitTime,
            OneShot = true
        };
        mixTimer.Timeout += FinishMixing;
        _mixingBar.MaxValue = BearBakery.MixerManager.MixWaitTime;
